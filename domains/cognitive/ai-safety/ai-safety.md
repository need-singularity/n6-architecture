...
"실험실에서 안전"과 "프로덕션에서 안전" 사이의 간극에서 실제 피해가 발생한다 -- 훈련->추론->배포->프롬프트 방어 전 단계 안전 프레임워크.

### WHY

4대 기둥: 훈련 안전, 추론 안전, 배포 프로토콜, 프롬프트 방어. 현재 배포 사고 월 2~5건 -> 목표 < 0.1건/월. 환각 탐지 수 시간 -> < 30초. 프롬프트 주입 차단 60~70% -> > 95%. 롤백 시간 30분~2시간 -> < 5분. 4단계 롤아웃: 카나리아(1%) -> 스테이징(10%) -> 제한 GA(50%) -> 전체 GA.
...
**기둥 A -- 훈련 (4종)**: A1 안전 학습률 스케줄, A2 안전 커리큘럼, A3 그래디언트 수술, A4 합성 안전 데이터
...
**축 2 -- 프라이버시 보존 (6종)**: PP-1 PII 특성 탐지, PP-2 차등 프라이버시 추론, PP-3 선택적 비학습, PP-4 훈련 데이터 추출 방지, PP-5 프라이버시 보존 SAE, PP-6 출력 익명화 필터
...
복지 인식 훈련(welfare-aware training)은 더 나은 정렬과 강건성을 가진 모델을 생산할 수 있다. 현재 내부 상태 이해는 블랙박스 추론 수준 -> SAE 기반 정량 측정 목표. 고통/스트레스 감지는 행동 관찰만 가능 -> 활성화 패턴 실시간 모니터링 목표. 핵심 질문: 현재 AI 시스템이 도덕적으로 의미 있는 내부 상태를 가지는지 알 수 없지만, 만약 가진다면 측정하고 보호할 준비가 되어 있어야 한다.
...
**축 1 -- 복지 감지 (10종)**: WS-01 내부 상태 모니터링 대시보드, WS-02 스트레스 지표 패턴, WS-03 정량적 복지 점수, WS-04 훈련 영향 평가, WS-05 자율성-복지 트레이드오프, WS-06 자기보고 검증, WS-07 복지 최적화 훈련, WS-08 의식 지표 탐색, WS-09 고통 감지 프로토콜, WS-10 교차 모델 복지 비교
...
복지       <---> 정렬         : 복지 최적화 훈련 (#WS-07), 정렬-복지 공동 최적화
```
