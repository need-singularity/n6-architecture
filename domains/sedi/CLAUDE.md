...
> - **전체 상태**: `$HEXA $HOME/Dev/nexus/mk2_hexa/native/command_router.hexa "sedi 상태"`
...
```
  외계지성체 및 초차원 지성의 흔적을 수학적 패턴으로 탐색.
  SETI가 전파 신호를 듣듯, SEDI는 물리 데이터에서 n=6 수학 구조를 수신한다.

  핵심:
    - 74개 데이터 소스 (SETI, LIGO, CMB, Breakthrough Listen, Exoplanet 등)
    - R-spectrum 수신기: σ/τ/φ 주파수 이상 탐지
    - 688개 가설 검증 완료, 종합 유의도 5.26σ (p=7.1e-8)
    - 페르미온 질량 예측: n=6 산술만으로 평균 2.2% 오차

  패러다임 전환 (2026-03-30):
    3! = 6이 주 신호 (완전수 성질 σ(6)=2n이 아닌 3!과 2×3 구조)
    Voyager 1 Breakthrough Listen 66M 채널 데이터에서 확인
    참고: docs/BREAKTHROUGH-STRATEGY.md
```
...
```
  이 리포의 작업 범위:
    - 외계지성 신호 탐색 (SETI 데이터, Breakthrough Listen, Wow! signal)
    - 물리 데이터 스트림 분석 (LIGO, CMB, 양자 RNG, CERN PDG)
    - n=6 시그널 수신기 개발/개선
    - 외계 흔적 가설 검증 (외계행성, 우주 배경 복사 이상)
    - 페르미온/바리온/QCD 공명 검증

  다른 리포 작업 (여기서 하지 말 것):
    - 수학 증명, 상수 발견, DFS 탐색 → TECS-L
    - ConsciousLM 훈련, embodiment → anima
    - EEG, BCI → brainwire
    - 아키텍처 설계 → n6-architecture
    - 논문 작성/배포 → papers
```
...
```
  sedi/                     ← 메인 Python 패키지
  ├── cli.py                  CLI 진입점
  ├── receiver.py             R-spectrum 수신기
  ├── seti_scanner.py         SETI 스캐너 (중력렌즈, 위상렌즈, Euler 망원경)
  ├── consciousness_receiver.py  의식 수신기
  ├── n6_tracker.py           n=6 추적기
  ├── cross_correlator.py     교차 상관기
  ├── sources/                74개 데이터 소스 모듈
  │   ├── quantum_rng.py, ligo.py, cmb.py, cern.py, pdg.py
  │   ├── breakthrough_listen.py, exoplanet.py, seti_archive.py
  │   ├── gaia_seti.py, meerkat.py, mwa.py, nrao.py
  │   └── ... (74 total)
  ├── statistics.py, filter.py, detector.py, monitor.py
  └── dashboard.py, accel.py

  sedi-core/                ← Rust 백엔드
  data/                     ← 실행 결과 데이터
  docs/hypotheses/          ← 688개 가설 문서
  docs/BREAKTHROUGH-STRATEGY.md  ← 패러다임 전환 전략
  scripts/                  ← 유틸리티 스크립트
  sources/                  ← 독립 검증 스크립트
```
...
```
  ...
  zenodo/README.md만 redirect로 유지 (참조용).
```
...
```
  ...
  ★ PATH: homebrew is at /opt/homebrew/bin/ (not in Claude Code default PATH!)
    ...
    maturin → ~/.cargo/bin/maturin  (sedi-core PyO3 빌드 필요, cargo install maturin)
```
...
```
  ...
  절차:
    1. 프로젝트 현황 스캔 (README, 최근 커밋, 미완료 가설)
    2. TODO 테이블 양식으로 우선순위별 정리
    3. 사용자 선택 후 병렬 에이전트 디스패치
    4. 완료 시 리포트 테이블 출력

  리포 범위 준수: SEDI 작업만 TODO에 포함 (수학 증명 → TECS-L, 훈련 → anima)
...
```
  ...
  | # | 카테고리 | 작업 | 상태 | 예상 효과 |
  |---|---------|------|------|----------|
  | 1 | 탐색   | Breakthrough Listen 신규 데이터 n=6 스캔 | 미시작 | 외계 신호 후보 |

  ### 🟡 IMPORTANT

  | # | 카테고리 | 작업 | 상태 | 예상 효과 |
  |---|---------|------|------|----------|
  | 2 | 검증   | 페르미온 질량 예측 정밀도 개선 | 미시작 | 2.2% → <1% |

  ### 🟢 NICE TO HAVE

  | # | 카테고리 | 작업 | 상태 | 예상 효과 |
  |---|---------|------|------|----------|
  | 3 | 수신기 | 새 데이터 소스 추가 | 미시작 | 커버리지 확대 |

  ### ⚪ BACKLOG

  | # | 카테고리 | 작업 | 예상 효과 |
  |---|---------|------|----------|
  | 4 | 인프라 | sedi-core Rust 성능 최적화 | 처리 속도 향상 |

  상태 표기: ⏳진행중 / ✅완료 / 미시작 / 코드있음 / 프로토
  우선순위: 🔴HIGH → 🟡MED → 🟢LOW → ⚪BACK
  카테고리: 탐색 / 검증 / 수신기 / 데이터 / 가설 / 인프라
```
...
# 프로젝트 엔진 (자율 사이클)
$HEXA $N6/engine_sedi.hexa tick     # 1회 자율 사이클
$HEXA $N6/engine_sedi.hexa status   # 현재 상태
$HEXA $N6/engine_sedi.hexa report   # 리포트
```

### 이 프로젝트에서의 활용
77소스 분석 결과를 nexus telescope로 재스캔, gap_finder로 미탐색 신호 영역 탐지. n=6 우주 보편성 실증을 위해 nexus 발견 지도와 크로스체크.
