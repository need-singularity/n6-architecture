...
requires:
  ...
  - to: sensors-iot
---
# 궁극의 양식과학 (HEXA-AQUA) — n=6 수질·사료·순환·생체 통합

## §1 WHY (이 기술이 당신의 삶을 바꾸는 방법)

어류 6대 생리 지표 (DO/pH/T/NH₃/NO₂/염분) = n=6 산술.
**aquaculture 영역의 3가지 기존 한계를 n=6 아키텍처가 동시 해결한다.**
...
| 효과 | 현재 | HEXA 이후 | 체감 변화 |
|------|------|-----------|----------|
| 생산성 kg/m³ | 15 | **60** | 체감: σ·sopfr=60 연결 |
| 사료효율 FCR | 2.0 | **1.0** | 체감: μ=1 연결 |
| 폐사율 % | 25 | **2** | 체감: φ=2 연결 |
| 수질 모니터 | 3 | **12** | 체감: σ=12 연결 |

**한 문장 요약**: 어류 6대 생리 지표 (DO/pH/T/NH₃/NO₂/염분) = n=6 산술 — n=6 완전수 아키텍처가 생산성 비약적 개선과 기존 한계 3가지를 동시에 해결한다.

### 일상이 되면

```
  [aquaculture] 데이터/자원/인프라가 n=6 구조로 정렬되면
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  [궁극의 양식과학 (HEXA-AQUA) 성능] 기존 vs HEXA 성능 비교                                        │
├──────────────────────────────────────────────────────────────────────────┤
│  생산성 kg/m³
│  기존   ███████░░░░░░░░░░░░░░░░░░░░░  15
│  HEXA   ████████████████████████████  60  (σ·sopfr=60)
...
|-------------|------|------|------|-----------|
| water-systems | 7 | 10 | +3 | 순환여과 |
| fish-biology | 7 | 10 | +3 | 어류 생리 |
| sensors-iot | 7 | 10 | +3 | 수질 σ=12 센서 |

3개 선행 도메인이 성숙되어야 통합 궁극의 양식과학 (HEXA-AQUA) 실현 가능. 현재는 부분 단계 (Mk.I~II).
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  궁극의 양식과학 (HEXA-AQUA) — 제원                                                        │
├──────────────────────────────────────────────────────────────────────────┤
│  본질          어류 6대 생리 지표 (DO/pH/T/NH₃/NO₂/염분) = n=6 산술
│  Core DOF      n = 6
...
궁극의 양식과학 (HEXA-AQUA) 가 n=6 구조로 성립하는지 stdlib 만으로 다층 검증. 주장된 설계 사양을 수론 유래 공식으로 cross-check.

### Testable Predictions (검증 가능한 예측 10건)

| # | 예측 | 공식 | 예측치 | Tier |
|---|------|------|--------|------|
| TP-1 | 생산성 최적값 | σ·sopfr/10 | 60 kg/m³ | 1 |
...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# 계열: aquaculture — HEXA n=6 정직성 검증 (stdlib only)
#
...
궁극의 양식과학 (HEXA-AQUA) 실제 실현 로드맵 — 각 Mk 단계마다 선행 도메인 성숙도 요구:

<details open>
<summary><b>Mk.V — 2050+ 전체 통합 (current target)</b></summary>

전체 통합. 어류 6대 생리 지표 (DO/pH/T/NH₃/NO₂/염분) = n=6 산술. 선행 3 도메인 모두 성숙 시 달성.
