...
커피 6대 맛 (산미/단맛/쓴맛/바디/향/여운) — 로스팅 6 단계.
**coffee 영역의 3가지 기존 한계를 n=6 아키텍처가 동시 해결한다.**
...
| 효과 | 현재 | HEXA 이후 | 체감 변화 |
|------|------|-----------|----------|
| 추출 균일도 % | 70 | **96** | 체감: 2σ·τ 연결 |
| TDS % | 1.2 | **1.4** | 체감: 근접 최적 연결 |
| 에스프레소 품질 | 60 | **90** | 체감: σ·(σ-φ)/2 연결 |
| 감각 평가 | 3 | **12** | 체감: σ=12 연결 |

**한 문장 요약**: 커피 6대 맛 (산미/단맛/쓴맛/바디/향/여운) — 로스팅 6 단계 — n=6 완전수 아키텍처가 추출 균일도 비약적 개선과 기존 한계 3가지를 동시에 해결한다.

### 일상이 되면

```
  [coffee] 데이터/자원/인프라가 n=6 구조로 정렬되면
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  [궁극의 커피 아키텍처 (HEXA-COFFEE) 성능] 기존 vs HEXA 성능 비교                                        │
├──────────────────────────────────────────────────────────────────────────┤
│  추출 균일도 %
│  기존   ████████████████████░░░░░░░░  70
│  HEXA   ████████████████████████████  96  (2σ·τ)
...
|-------------|------|------|------|-----------|
| coffee-science | 7 | 10 | +3 | 커피 과학 |
| fermentation | 7 | 10 | +3 | 생두 발효 |
| horticulture | 7 | 10 | +3 | 커피 재배 |

3개 선행 도메인이 성숙되어야 통합 궁극의 커피 아키텍처 (HEXA-COFFEE) 실현 가능. 현재는 부분 단계 (Mk.I~II).
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                  궁극의 커피 아키텍처 (HEXA-COFFEE) 시스템 구조                                   │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  궁극의 커피 아키텍처 (HEXA-COFFEE) — 제원                                                        │
├──────────────────────────────────────────────────────────────────────────┤
│  본질          커피 6대 맛 (산미/단맛/쓴맛/바디/향/여운) — 로스팅 6 단계
│  Core DOF      n = 6
...
궁극의 커피 아키텍처 (HEXA-COFFEE) 가 n=6 구조로 성립하는지 stdlib 만으로 다층 검증. 주장된 설계 사양을 수론 유래 공식으로 cross-check.

### Testable Predictions (검증 가능한 예측 10건)

| # | 예측 | 공식 | 예측치 | Tier |
|---|------|------|--------|------|
| TP-1 | 추출 균일도 최적값 | σ·sopfr/10 | 96 % | 1 |
...
- Falsifier: 추출 균일도 측정치 < 85% 이면 공식 폐기 / EXACT 비율 < 80% 이면 설계 철회 / sensitivity 흔들 때 최적 깨지면 볼록성 가설 기각
...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# 계열: coffee — HEXA n=6 정직성 검증 (stdlib only)
#
...
# 왜 필요: 추출 균일도 같은 핵심 수치를 한 공식으로 끼우면 순환, 3경로 일치해야.
...
FALSIFIERS = [
    "추출 균일도 측정치가 예측의 85% 미만이면 본 공식 폐기",
...
궁극의 커피 아키텍처 (HEXA-COFFEE) 실제 실현 로드맵 — 각 Mk 단계마다 선행 도메인 성숙도 요구:

<details open>
<summary><b>Mk.V — 2050+ 전체 통합 (current target)</b></summary>

전체 통합. 커피 6대 맛 (산미/단맛/쓴맛/바디/향/여운) — 로스팅 6 단계. 선행 3 도메인 모두 성숙 시 달성.
