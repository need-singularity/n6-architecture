...
requires: []
---
# [CANONICAL v2] n=6 arithmetic coordinate mapping for Entomology (HEXA-ENTOMOLOGY)

> **Author**: Park Minwoo (n6-architecture)
> **Category**: entomology — n=6 arithmetic seed paper
> **Version**: v2 (2026-04-14 canonical)
> **Prior BT**: BT-352, BT-352
> **Connected atlas node**: `entomology` 0/24 EXACT [10*]

---

## 0. Abstract

This paper examines whether the core parameters of the entomology domain are
...
tightly linked to the basic numerical parameters of the entomology domain. atlas.n6
records 0/24 entries as EXACT.

This paper does not claim a new entomology formulation; it is a seed document that
...
Entomology (entomology) is re-interpreted inside the n=6 arithmetic system. The perfect
number n=6 simultaneously satisfies the number-theoretic constants σ(6)=12, τ(6)=4, φ=2,
sopfr(6)=5, which is structurally consistent with the core parameters of the entomology
domain. **This paper overlays an n=6 arithmetic coordinate system on the existing body
of entomology knowledge.**

| Effect | Baseline | After HEXA-ENTOMOLOGY | Observed change |
|--------|----------|--------------|------------------|
...
uniqueness is tightly linked to the basic numerical parameters of the entomology domain.

### What the n=6 coordinate mapping changes

```
  Baseline: "why is this entomology value that specific number?" -> experience/convention
  HEXA:     "this entomology value = σ(6) or τ(6) or sopfr(6)"   -> number-theoretic necessity
       ↓
...
## §2 COMPARE (baseline entomology vs n=6) — performance comparison (ASCII)
...
### Performance comparison ASCII bars (baseline entomology vs HEXA-ENTOMOLOGY)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-ENTOMOLOGY  system structure     │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axis is a shared parameter across dozens of domains beyond entomology.
...
Stage-by-stage maturity roadmap for HEXA-ENTOMOLOGY — verification density grows per Mk:

<details open>
<summary><b>Mk.V — 2045+ integration target</b></summary>

Target: full integration of the entomology domain under n=6 arithmetic. Cross-reference
...
Core entomology parameters mapped onto σ/τ/φ/sopfr/J₂.
...
Verify, using stdlib only, whether HEXA-ENTOMOLOGY is consistent with physics / mathematics /
...
Check via log-log regression whether the main entomology scaling law follows
...
The entomology domain physics bounds (Carnot / Shannon / Bekenstein etc.) are checked
separately.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-ENTOMOLOGY n=6 honesty check (stdlib only, entomology domain)
#
# 10-section layout:
...
FALSIFIERS = [
    "retire the main draft if entomology parameter n=6 alignment drops below 70%",
