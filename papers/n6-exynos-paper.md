...
> **Connected atlas node**: `exynos` 32/32 EXACT [10*]

---

## 0. Abstract

This paper examines whether the core parameters of the exynos domain are
...
tightly linked to the basic numerical parameters of the exynos domain. atlas.n6
records 32/32 entries as EXACT.

This paper does not claim a new exynos formulation; it is a seed document that
...
Exynos (exynos) is re-interpreted inside the n=6 arithmetic system. The perfect
number n=6 simultaneously satisfies the number-theoretic constants σ(6)=12, τ(6)=4, φ=2,
sopfr(6)=5, which is structurally consistent with the core parameters of the exynos
domain. **This paper overlays an n=6 arithmetic coordinate system on the existing body
of exynos knowledge.**

| Effect | Baseline | After HEXA-EXYNOS | Observed change |
|--------|----------|--------------|------------------|
...
uniqueness is tightly linked to the basic numerical parameters of the exynos domain.

### What the n=6 coordinate mapping changes

```
  Baseline: "why is this exynos value that specific number?" -> experience/convention
  HEXA:     "this exynos value = σ(6) or τ(6) or sopfr(6)"   -> number-theoretic necessity
       ↓
...
## §2 COMPARE (baseline exynos vs n=6) — performance comparison (ASCII)
...
### Performance comparison ASCII bars (baseline exynos vs HEXA-EXYNOS)
...
4. **Domain overlap**: the σ=12 axis is a shared parameter across dozens of domains beyond exynos.
...
Stage-by-stage maturity roadmap for HEXA-EXYNOS — verification density grows per Mk:

<details open>
<summary><b>Mk.V — 2045+ integration target</b></summary>

Target: full integration of the exynos domain under n=6 arithmetic. Cross-reference
...
Core exynos parameters mapped onto σ/τ/φ/sopfr/J₂.
...
Verify, using stdlib only, whether HEXA-EXYNOS is consistent with physics / mathematics /
...
Check via log-log regression whether the main exynos scaling law follows
...
The exynos domain physics bounds (Carnot / Shannon / Bekenstein etc.) are checked
separately.

### §7.6 CHI2 — H₀: n=6 coincidence hypothesis p-value
Compute 32/32 EXACT under H₀ (random match) -> p-value.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-EXYNOS n=6 honesty check (stdlib only, exynos domain)
#
# 10-section layout:
...
FALSIFIERS = [
    "retire the main draft if exynos parameter n=6 alignment drops below 70%",
    "retire the uniqueness claim if sigma(n)*phi(n) = n*tau(n) is observed at n != 6",
    "downgrade to Mk.I if atlas 32/32 EXACT re-measurement falls below 70%",
