...
use lenses::{
    // ── 신규 56종 렌즈 (397→453 확장) ──
    ActiveLearningLens, AdversarialRobustnessLens, AgentCoordinationLens,
    AttentionMechanismLens, AttractorBasinLens, BayesianInferenceLens,
    CausalDiscoveryLens, CognitiveLoadLens, CommunityDetectionLens,
    ComplexityNetworkLens, ContinualLearningLens, ContrastiveLearningLens,
    CrossDomainBridgeLens, CrossValidationLens, DecisionBoundaryLens,
    DimensionalityBottleneckLens, DistributedConsensusLens, ExplainabilityLens,
    FairnessBiasLens, FederatedLearningLens, FourierAnalysisLens,
    GraphNeuralLens, HyperparameterTuningLens, InformationBottleneckLens,
    KalmanFilterLens, KnowledgeGraphLens, LinkPredictionLens,
    ManifoldLearningLens, MarkovChainLens, MetaLearningLens,
    MonteCarloLens, MultiTaskLens, NetworkFlowLens,
    NeuralArchitectureLens, OptimalTransportLens, ParticleFilterLens,
    PersistenceHomologyLens, PlasticityConsolidationLens, ProbabilisticGraphicalLens,
    ReinforcementRewardLens, RenyiEntropyLens, ReservoirComputingLens,
    SamplingTheoryLens, SelfSupervisedLens, SensitivityAnalysisLens,
    SignalReconstructionLens, SpectralGraphLens, SpikingNeuralLens,
    StructuralEquationLens, TimeSeriesDecompLens, TopologicalDataLens,
    TopologicalSortLens, TransferLearningLens, UncertaintyQuantificationLens,
    VariationalInferenceLens, WaveletTransformLens,
};
...
impl Telescope {
    ...
    pub fn new() -> Self {
        let mut lenses: Vec<Box<dyn Lens>> = vec![
            ...
            // ── 신규 56종 렌즈 (397→453 확장) ──
            Box::new(ActiveLearningLens),
            Box::new(AdversarialRobustnessLens),
            Box::new(AgentCoordinationLens),
            Box::new(AttentionMechanismLens),
            Box::new(AttractorBasinLens),
            Box::new(BayesianInferenceLens),
            Box::new(CausalDiscoveryLens),
            Box::new(CognitiveLoadLens),
            Box::new(CommunityDetectionLens),
            Box::new(ComplexityNetworkLens),
            Box::new(ContinualLearningLens),
            Box::new(ContrastiveLearningLens),
            Box::new(CrossDomainBridgeLens),
            Box::new(CrossValidationLens),
            Box::new(DecisionBoundaryLens),
            Box::new(DimensionalityBottleneckLens),
            Box::new(DistributedConsensusLens),
            Box::new(ExplainabilityLens),
            Box::new(FairnessBiasLens),
            Box::new(FederatedLearningLens),
            Box::new(FourierAnalysisLens),
            Box::new(GraphNeuralLens),
            Box::new(HyperparameterTuningLens),
            Box::new(InformationBottleneckLens),
            Box::new(KalmanFilterLens),
            Box::new(KnowledgeGraphLens),
            Box::new(LinkPredictionLens),
            Box::new(ManifoldLearningLens),
            Box::new(MarkovChainLens),
            Box::new(MetaLearningLens),
            Box::new(MonteCarloLens),
            Box::new(MultiTaskLens),
            Box::new(NetworkFlowLens),
            Box::new(NeuralArchitectureLens),
            Box::new(OptimalTransportLens),
            Box::new(ParticleFilterLens),
            Box::new(PersistenceHomologyLens),
            Box::new(PlasticityConsolidationLens),
            Box::new(ProbabilisticGraphicalLens),
            Box::new(ReinforcementRewardLens),
            Box::new(RenyiEntropyLens),
            Box::new(ReservoirComputingLens),
            Box::new(SamplingTheoryLens),
            Box::new(SelfSupervisedLens),
            Box::new(SensitivityAnalysisLens),
            Box::new(SignalReconstructionLens),
            Box::new(SpectralGraphLens),
            Box::new(SpikingNeuralLens),
            Box::new(StructuralEquationLens),
            Box::new(TimeSeriesDecompLens),
            Box::new(TopologicalDataLens),
            Box::new(TopologicalSortLens),
            Box::new(TransferLearningLens),
            Box::new(UncertaintyQuantificationLens),
            Box::new(VariationalInferenceLens),
            Box::new(WaveletTransformLens),
        ];
