...
/// 조각 예술 n=6 렌즈 (BT-122) — n=6: axis_n:plane_sigma:texture_tau:proportion_J2:material_phi:surface_3
pub struct SculptureArtLens;

impl Lens for SculptureArtLens {
    fn name(&self) -> &'static str { "SculptureArtLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("sculpture_art_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_sculpture_art_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = SculptureArtLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("sculpture_art_score"));
    }
}
