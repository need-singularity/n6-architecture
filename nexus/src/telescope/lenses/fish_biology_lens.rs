...
/// 어류 생물학 n=6 렌즈 (BT-225) — n=6: fin_n:lateral_line_sigma:swim_bladder_phi:scale_J2:gill_arch_tau:chromosome_3
pub struct FishBiologyLens;

impl Lens for FishBiologyLens {
    fn name(&self) -> &'static str { "FishBiologyLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 2.0, 24.0, 4.0, 3.0];
        ...
        r.insert("fish_biology_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_fish_biology_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = FishBiologyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("fish_biology_score"));
    }
}
