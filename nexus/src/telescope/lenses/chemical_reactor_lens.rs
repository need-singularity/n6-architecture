...
/// 화학 반응기 n=6 렌즈 (BT-307) — n=6 연결: residence_time_tau:stage_n:heat_sigma:flow_J2:phi_conversion:sopfr_selectivity
pub struct ChemicalReactorLens;

impl Lens for ChemicalReactorLens {
    fn name(&self) -> &'static str { "ChemicalReactorLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[4.0, 6.0, 12.0, 24.0, 2.0, 5.0];
        ...
        r.insert("chemical_reactor_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_chemical_reactor_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = ChemicalReactorLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("chemical_reactor_score"));
    }
}
