...
/// 치의학 n=6 렌즈 (BT-286) — n=6: quadrant_tau:tooth_J2:root_n:canal_phi:occlusal_sigma:implant_3
pub struct DentistryLens;

impl Lens for DentistryLens {
    fn name(&self) -> &'static str { "DentistryLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[4.0, 24.0, 6.0, 2.0, 12.0, 3.0];
        ...
        r.insert("dentistry_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_dentistry_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = DentistryLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("dentistry_score"));
    }
}
