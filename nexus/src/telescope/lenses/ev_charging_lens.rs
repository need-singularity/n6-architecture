...
/// EV 충전 n=6 렌즈 (BT-206,BT-288) — n=6 연결: charging_kw:6.0:n_phase:tau_port:dc_fast_288kw:hbm_sigma
pub struct EvChargingLens;

impl Lens for EvChargingLens {
    fn name(&self) -> &'static str { "EvChargingLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 4.0, 288.0, 48.0, 12.0, 24.0];
        ...
        r.insert("ev_charging_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_ev_charging_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = EvChargingLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("ev_charging_score"));
    }
}
