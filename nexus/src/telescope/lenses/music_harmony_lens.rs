...
/// 음악 화성 n=6 렌즈 — BT-108, BT-135
/// n=6 연결: 12음계=sigma, 옥타브=phi=2, 협화음=div(6)
pub struct MusicHarmonyLens;

impl Lens for MusicHarmonyLens {
    fn name(&self) -> &'static str { "MusicHarmonyLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let domain_consts: &[(f64, &str)] = &[
    (12.0, "chromatic_sigma"),
    (2.0, "octave_phi"),
    (6.0, "n_harmony"),
    (4.0, "tau_chord"),
    (24.0, "J2_tempo"),
    (3.0, "triad"),        ];
        ...
        r.insert("music_harmony_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_music_harmony_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = MusicHarmonyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("music_harmony_score"));
    }
}
