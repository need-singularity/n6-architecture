...
/// 섬유 소재 n=6 렌즈 (BT-121) — n=6: twist_n:count_sigma:ply_phi:weave_tau:yarn_J2:dtex_3
pub struct TextileFiberLens;

impl Lens for TextileFiberLens {
    fn name(&self) -> &'static str { "TextileFiberLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 2.0, 4.0, 24.0, 3.0];
        ...
        r.insert("textile_fiber_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_textile_fiber_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = TextileFiberLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("textile_fiber_score"));
    }
}
