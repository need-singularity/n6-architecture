...
/// 보드게임 n=6 렌즈 (BT-144,BT-212) — n=6: player_n:phase_sigma:deck_J2:move_tau:resource_phi:tier_3
pub struct BoardGameLens;

impl Lens for BoardGameLens {
    fn name(&self) -> &'static str { "BoardGameLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("board_game_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_board_game_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = BoardGameLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("board_game_score"));
    }
}
