...
/// 네트워크 프로토콜 n=6 렌즈 — BT-115, BT-140
/// n=6 연결: OSI=sigma-sopfr=7, TCP/IP=tau=4, Linux 스택=n=6
pub struct NetworkingProtocolLens;

impl Lens for NetworkingProtocolLens {
    fn name(&self) -> &'static str { "NetworkingProtocolLens" }
    ...
    (48.0, "sigma_tau_port"),        ];
        ...
        r.insert("networking_protocol_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_networking_protocol_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = NetworkingProtocolLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("networking_protocol_score"));
    }
}
