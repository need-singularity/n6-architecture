...
/// 양자회로 n=6 렌즈 — BT-195
/// n=6 연결: 큐비트 레지스터=n=6, 게이트레이어=sigma=12, 오류정정=J2=24, 토폴로지=tau=4
pub struct QuantumCircuitLens;

impl Lens for QuantumCircuitLens {
    fn name(&self) -> &'static str { "QuantumCircuitLens" }
    ...
    (48.0, "sigma_tau_fid"),        ];
        ...
        r.insert("quantum_circuit_score".to_string(), vec![score]);
...
mod tests {
    ...
    fn test_quantum_circuit_basic() {
        ...
        let result = QuantumCircuitLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("quantum_circuit_score"));
    }
}
