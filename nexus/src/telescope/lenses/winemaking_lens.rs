...
/// 포도주 제조 n=6 렌즈 (BT-192) — n=6: fermentation_stage_n:ph_tau:brix_sigma:phenol_J2:oak_phi:vintage_3
pub struct WinemakingLens;

impl Lens for WinemakingLens {
    fn name(&self) -> &'static str { "WinemakingLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("winemaking_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_winemaking_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = WinemakingLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("winemaking_score"));
    }
}
