...
/// 핵반응로 n=6 렌즈 (BT-98,BT-291) — n=6 연결: fuel_rods_6:enrichment_4:coolant_loop_6:cycle_18:output_sigma
pub struct NuclearReactorLens;

impl Lens for NuclearReactorLens {
    fn name(&self) -> &'static str { "NuclearReactorLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 4.0, 18.0, 12.0, 24.0, 3.0];
        ...
        r.insert("nuclear_reactor_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_nuclear_reactor_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = NuclearReactorLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("nuclear_reactor_score"));
    }
}
