...
/// 사진 렌즈 n=6 렌즈 (BT-222) — n=6: aperture_n:element_sigma:group_tau:coating_phi:bokeh_J2:focal_3
pub struct PhotographyLensLens;

impl Lens for PhotographyLensLens {
    fn name(&self) -> &'static str { "PhotographyLensLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 4.0, 2.0, 24.0, 3.0];
        ...
        r.insert("photography_lens_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_photography_lens_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = PhotographyLensLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("photography_lens_score"));
    }
}
