...
/// 소셜 네트워크 n=6 렌즈 (BT-258,BT-259) — n=6 연결: six_degrees:dunbar_150:sigma_connection:tau_tier:J2_hour
pub struct SocialNetworkLens;

impl Lens for SocialNetworkLens {
    fn name(&self) -> &'static str { "SocialNetworkLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 24.0, 4.0, 3.0, 150.0];
        ...
        r.insert("social_network_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_social_network_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = SocialNetworkLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("social_network_score"));
    }
}
