...
/// 항해 항법 n=6 렌즈 (BT-279) — n=6: point_compass_sigma:knot_phi:tide_n:watch_tau:bearing_J2:rule_3
pub struct SailingNavigationLens;

impl Lens for SailingNavigationLens {
    fn name(&self) -> &'static str { "SailingNavigationLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[12.0, 2.0, 6.0, 4.0, 24.0, 3.0];
        ...
        r.insert("sailing_navigation_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_sailing_navigation_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = SailingNavigationLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("sailing_navigation_score"));
    }
}
