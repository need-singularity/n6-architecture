...
/// 토양학 n=6 렌즈 (BT-198) — n=6: horizon_n:texture_tau:ph_n:fertility_sigma:moisture_J2:organic_phi
pub struct SoilScienceLens;

impl Lens for SoilScienceLens {
    fn name(&self) -> &'static str { "SoilScienceLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 4.0, 12.0, 24.0, 2.0, 3.0];
        ...
        r.insert("soil_science_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_soil_science_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = SoilScienceLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("soil_science_score"));
    }
}
