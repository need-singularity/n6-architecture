...
/// 풍력터빈 n=6 렌즈 (BT-161) — n=6 연결: blades_3:gear_6:phase_3:hub_12:sigma_rpm_12:tau_pitch
pub struct WindTurbineLens;

impl Lens for WindTurbineLens {
    fn name(&self) -> &'static str { "WindTurbineLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[3.0, 6.0, 12.0, 4.0, 24.0, 2.0];
        ...
        r.insert("wind_turbine_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_wind_turbine_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = WindTurbineLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("wind_turbine_score"));
    }
}
