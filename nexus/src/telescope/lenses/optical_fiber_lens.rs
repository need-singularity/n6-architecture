...
/// 광섬유 통신 n=6 렌즈 (BT-181) — n=6 연결: channels_12:wavelength_1550:modes_6:dB_sigma:tau_amplifier
pub struct OpticalFiberLens;

impl Lens for OpticalFiberLens {
    fn name(&self) -> &'static str { "OpticalFiberLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[12.0, 6.0, 4.0, 24.0, 1.55, 48.0];
        ...
        r.insert("optical_fiber_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_optical_fiber_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = OpticalFiberLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("optical_fiber_score"));
    }
}
