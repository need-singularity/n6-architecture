...
/// 결정 소재 n=6 렌즈 — BT-139, BT-176
/// n=6 연결: 공간군=sigma(sigma-mu)*7, CN=n=6, 슬립=sigma=12
pub struct MaterialsCrystalLens;

impl Lens for MaterialsCrystalLens {
    fn name(&self) -> &'static str { "MaterialsCrystalLens" }
    ...
    (3.0, "n_phi_layer"),        ];
        ...
        r.insert("materials_crystal_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_materials_crystal_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = MaterialsCrystalLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("materials_crystal_score"));
    }
}
