...
/// 법과학 n=6 렌즈 (BT-131) — n=6: dna_loci_sigma:fingerprint_ridge_n:blood_pattern_tau:trace_J2:tool_mark_phi:tier_3
pub struct ForensicScienceLens;

impl Lens for ForensicScienceLens {
    fn name(&self) -> &'static str { "ForensicScienceLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[12.0, 6.0, 4.0, 24.0, 2.0, 3.0];
        ...
        r.insert("forensic_science_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_forensic_science_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = ForensicScienceLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("forensic_science_score"));
    }
}
