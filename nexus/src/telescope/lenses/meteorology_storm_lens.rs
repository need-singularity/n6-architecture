...
/// 기상 폭풍 n=6 렌즈 (BT-218) — n=6: saffir_n:pressure_drop_sigma:wind_J2:rain_tau:eye_phi:spiral_3
pub struct MeteorologyStormLens;

impl Lens for MeteorologyStormLens {
    fn name(&self) -> &'static str { "MeteorologyStormLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 24.0, 4.0, 2.0, 3.0];
        ...
        r.insert("meteorology_storm_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_meteorology_storm_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = MeteorologyStormLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("meteorology_storm_score"));
    }
}
