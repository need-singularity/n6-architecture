...
/// 스포츠 생체역학 n=6 렌즈 (BT-148) — n=6: joint_dof_n:muscle_group_sigma:phase_tau:force_J2:cycle_phi:power_3
pub struct SportsBiomechLens;

impl Lens for SportsBiomechLens {
    fn name(&self) -> &'static str { "SportsBiomechLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("sports_biomech_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_sports_biomech_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = SportsBiomechLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("sports_biomech_score"));
    }
}
