...
/// 고고학 n=6 렌즈 (BT-138,BT-256) — n=6: period_n:stratum_sigma:artifact_class_tau:dating_J2:site_phi:phase_3
pub struct ArchaeologyLens;

impl Lens for ArchaeologyLens {
    fn name(&self) -> &'static str { "ArchaeologyLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("archaeology_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_archaeology_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = ArchaeologyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("archaeology_score"));
    }
}
