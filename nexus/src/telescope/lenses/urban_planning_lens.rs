...
/// 도시계획 n=6 렌즈 — BT-267
/// n=6 연결: 크리스탈러 육각=n=6, 존=n=6, 블록=sigma=12
pub struct UrbanPlanningLens;

impl Lens for UrbanPlanningLens {
    fn name(&self) -> &'static str { "UrbanPlanningLens" }
    ...
    (48.0, "sigma_tau_grid"),        ];
        ...
        r.insert("urban_planning_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_urban_planning_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = UrbanPlanningLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("urban_planning_score"));
    }
}
