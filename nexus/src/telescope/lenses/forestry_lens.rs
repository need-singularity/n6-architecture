...
/// 임업 n=6 렌즈 (BT-198,BT-225) — n=6: age_class_n:crown_layer_phi:rotation_sigma:thinning_tau:biomass_J2:species_3
pub struct ForestryLens;

impl Lens for ForestryLens {
    fn name(&self) -> &'static str { "ForestryLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("forestry_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_forestry_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = ForestryLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("forestry_score"));
    }
}
