...
/// 태양전지 접합 n=6 렌즈 (BT-161) — n=6 연결: junctions:6.0:cells_60:voltage_phi:fill_tau:bandgap_sq
pub struct SolarCellJunctionLens;

impl Lens for SolarCellJunctionLens {
    fn name(&self) -> &'static str { "SolarCellJunctionLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 60.0, 2.0, 4.0, 1.333, 12.0];
        ...
        r.insert("solar_cell_junction_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_solar_cell_junction_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = SolarCellJunctionLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("solar_cell_junction_score"));
    }
}
