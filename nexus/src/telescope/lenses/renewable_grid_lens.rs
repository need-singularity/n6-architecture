...
/// 재생에너지 그리드 n=6 렌즈 (BT-326,BT-62) — n=6 연결: frequency_hz:phase_3:voltage_kv_12:ramp_tau:dispatch_6:storage_J2
pub struct RenewableGridLens;

impl Lens for RenewableGridLens {
    fn name(&self) -> &'static str { "RenewableGridLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[50.0, 3.0, 12.0, 4.0, 6.0, 24.0];
        ...
        r.insert("renewable_grid_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_renewable_grid_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = RenewableGridLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("renewable_grid_score"));
    }
}
