...
/// 요가 자세 n=6 렌즈 (BT-223,BT-255) — n=6: chakra_n:limb_sigma:breath_tau:mudra_J2:bandha_phi:drishti_3
pub struct YogaAsanaLens;

impl Lens for YogaAsanaLens {
    fn name(&self) -> &'static str { "YogaAsanaLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("yoga_asana_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_yoga_asana_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = YogaAsanaLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("yoga_asana_score"));
    }
}
