...
use nexus::telescope::anima_lenses::anima_consciousness_lens_entries;
use nexus::telescope::consensus::{weighted_consensus, ConsensusLevel};
...
use nexus::telescope::accel_lenses_d::accel_humanities_lens_entries;
use nexus::telescope::domain_combos::default_combos;
use nexus::telescope::lens_trait::{Lens, LensResult};
use nexus::telescope::lenses::{
...
use nexus::telescope::registry::{LensCategory, LensEntry, LensRegistry};
use nexus::telescope::shared_data::SharedData;
use nexus::telescope::tier::TieredScanner;
use nexus::telescope::Telescope;
...
fn test_telescope_scan_all() {
    ...
    assert!(telescope.lens_count() >= 179, "Expected at least 179 lenses, got {}", telescope.lens_count());
}
...
fn test_registry_core_count() {
    ...
    assert_eq!(reg.len(), 1034, "Total registry size should be 1034 after new()");
}
...
fn test_registry_by_category() {
    ...
    // Extended 렌즈 수 (마케팅/디지털트윈/발효/시계 4종 추가로 1011)
    assert_eq!(reg.by_category(LensCategory::Extended).len(), 1011);
    ...
    assert_eq!(reg.by_category(LensCategory::Extended).len(), 1012);
    assert_eq!(reg.len(), 1035);
}
...
fn test_global_lens_name_uniqueness() {
    ...
    for e in nexus::telescope::quantum_lenses::quantum_topology_lens_entries() {
        all_names.push(e.name.clone());
    }
    for e in nexus::telescope::physics_deep_lenses::physics_deep_lens_entries() {
        all_names.push(e.name.clone());
    }
    for e in nexus::telescope::frontier_lenses::frontier_lens_entries() {
        all_names.push(e.name.clone());
    }

    let total = all_names.len();
    assert_eq!(total, 1022, "Total = 23+58+75+103+88+100+58+57+55+63+273+49+8 = 1022");
...
fn test_registry_total_411() {
    let reg = LensRegistry::new();
    assert_eq!(reg.len(), 1034, "Registry should have 1034 lenses total");

    let extended = reg.by_category(LensCategory::Extended);
    assert_eq!(extended.len(), 1011, "Extended category should have 1011 lenses");
}
...
fn test_telescope_has_all_24_lenses() {
    let telescope = Telescope::new();
    // 179 dedicated + GenericLens auto-instantiation from registry
    assert!(
        telescope.lens_count() >= 179,
        "Telescope::new() should register at least 179 dedicated lenses, got {}",
...
fn test_telescope_scan_all_24() {
    ...
    assert!(
        results.len() >= 179,
        "scan_all should return results for at least 179 lenses, got {}",
        results.len()
    );
