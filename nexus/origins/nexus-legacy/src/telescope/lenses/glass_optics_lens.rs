...
/// 유리 광학 n=6 렌즈 (BT-189) — n=6: refractive_layers_n:dispersion_phi:thickness_sigma:abbe_tau:coating_J2:index_n
pub struct GlassOpticsLens;

impl Lens for GlassOpticsLens {
    fn name(&self) -> &'static str { "GlassOpticsLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("glass_optics_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_glass_optics_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = GlassOpticsLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("glass_optics_score"));
    }
}
