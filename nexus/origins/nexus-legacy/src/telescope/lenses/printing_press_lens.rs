...
/// 인쇄 공정 n=6 렌즈 (BT-226) — n=6: color_channel_tau:screen_angle_n:resolution_sigma:dot_gain_phi:halftone_J2:cmyk_4
pub struct PrintingPressLens;

impl Lens for PrintingPressLens {
    fn name(&self) -> &'static str { "PrintingPressLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[4.0, 6.0, 12.0, 2.0, 24.0, 3.0];
        ...
        r.insert("printing_press_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_printing_press_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = PrintingPressLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("printing_press_score"));
    }
}
