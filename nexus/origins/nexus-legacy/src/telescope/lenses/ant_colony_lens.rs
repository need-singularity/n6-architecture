...
/// 개미집단 최적화 n=6 렌즈 (BT-225) — n=6 연결: pheromone_decay:ants_6:path_sigma:tau_update:J2_iter:n_best
pub struct AntColonyLens;

impl Lens for AntColonyLens {
    fn name(&self) -> &'static str { "AntColonyLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("ant_colony_score".to_string(), vec![score]);
...
mod tests {
    ...
    fn test_ant_colony_basic() {
        ...
        let result = AntColonyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("ant_colony_score"));
    }
}
