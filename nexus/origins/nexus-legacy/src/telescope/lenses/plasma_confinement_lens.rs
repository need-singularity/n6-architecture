...
/// 플라즈마 가둠 n=6 렌즈 — BT-317, BT-243
/// n=6 연결: TF코일=3n=18, PF=n=6, q=tau/phi=2, 가열=tau=4
pub struct PlasmaConfinementLens;

impl Lens for PlasmaConfinementLens {
    fn name(&self) -> &'static str { "PlasmaConfinementLens" }
    ...
    (24.0, "J2_power"),        ];
        ...
        r.insert("plasma_confinement_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_plasma_confinement_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = PlasmaConfinementLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("plasma_confinement_score"));
    }
}
