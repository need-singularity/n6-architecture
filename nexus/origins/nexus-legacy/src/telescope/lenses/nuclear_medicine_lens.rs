...
/// 핵의학 n=6 렌즈 (BT-173,BT-128) — n=6: half_life_tau:energy_sigma:dose_J2:organ_n:scan_phi:protocol_3
pub struct NuclearMedicineLens;

impl Lens for NuclearMedicineLens {
    fn name(&self) -> &'static str { "NuclearMedicineLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[4.0, 12.0, 24.0, 6.0, 2.0, 3.0];
        ...
        r.insert("nuclear_medicine_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_nuclear_medicine_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = NuclearMedicineLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("nuclear_medicine_score"));
    }
}
