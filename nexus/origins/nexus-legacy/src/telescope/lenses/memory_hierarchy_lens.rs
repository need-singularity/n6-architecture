...
/// 메모리 계층 n=6 렌즈 — BT-142, BT-180
/// n=6 연결: tau=4 레벨, sigma=12 캐시셋, J2=24 채널, 288GB HBM
pub struct MemoryHierarchyLens;

impl Lens for MemoryHierarchyLens {
    fn name(&self) -> &'static str { "MemoryHierarchyLens" }
    ...
    (64.0, "cache_line"),        ];
        ...
        r.insert("memory_hierarchy_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_memory_hierarchy_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = MemoryHierarchyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("memory_hierarchy_score"));
    }
}
