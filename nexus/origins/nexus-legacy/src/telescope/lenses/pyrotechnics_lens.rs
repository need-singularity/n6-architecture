...
/// 불꽃 화학 n=6 렌즈 (BT-118,BT-307) — n=6: oxidizer_phi:fuel_n:metal_sigma:color_tau:burst_J2:delay_3
pub struct PyrotechnicsLens;

impl Lens for PyrotechnicsLens {
    fn name(&self) -> &'static str { "PyrotechnicsLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[2.0, 6.0, 12.0, 4.0, 24.0, 3.0];
        ...
        r.insert("pyrotechnics_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_pyrotechnics_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = PyrotechnicsLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("pyrotechnics_score"));
    }
}
