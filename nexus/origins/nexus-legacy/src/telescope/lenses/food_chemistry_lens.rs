...
/// 식품 화학 n=6 렌즈 — BT-192, BT-341
/// n=6 연결: 포도당=C6H12O6, 비타민B6=6, 아미노산 분류=n=6
pub struct FoodChemistryLens;

impl Lens for FoodChemistryLens {
    fn name(&self) -> &'static str { "FoodChemistryLens" }
    ...
    (24.0, "J2_minerals"),        ];
        ...
        r.insert("food_chemistry_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_food_chemistry_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = FoodChemistryLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("food_chemistry_score"));
    }
}
