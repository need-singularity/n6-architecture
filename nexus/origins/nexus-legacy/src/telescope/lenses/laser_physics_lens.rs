...
/// 레이저 물리 n=6 렌즈 (BT-145,BT-189) — n=6: mode_n:cavity_sigma:pulse_tau:coherence_J2:pumping_phi:beam_3
pub struct LaserPhysicsLens;

impl Lens for LaserPhysicsLens {
    fn name(&self) -> &'static str { "LaserPhysicsLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("laser_physics_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_laser_physics_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = LaserPhysicsLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("laser_physics_score"));
    }
}
