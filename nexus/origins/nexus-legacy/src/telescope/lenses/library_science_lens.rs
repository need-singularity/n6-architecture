...
/// 도서관학 n=6 렌즈 (BT-197) — n=6: dewey_division_n:lc_class_sigma:marc_field_tau:isbn_J2:metadata_phi:tier_3
pub struct LibraryScienceLens;

impl Lens for LibraryScienceLens {
    fn name(&self) -> &'static str { "LibraryScienceLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("library_science_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_library_science_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = LibraryScienceLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("library_science_score"));
    }
}
