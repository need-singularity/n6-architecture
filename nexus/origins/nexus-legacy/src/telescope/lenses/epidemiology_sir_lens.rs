...
/// 역학 SIR 모델 n=6 렌즈 (BT-204) — n=6 연결: compartments_n:R0_tau:latency_sigma:immunity_J2:herd_phi:wave_3
pub struct EpidemiologySirLens;

impl Lens for EpidemiologySirLens {
    fn name(&self) -> &'static str { "EpidemiologySirLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("epidemiology_sir_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_epidemiology_sir_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = EpidemiologySirLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("epidemiology_sir_score"));
    }
}
