...
/// 팬데믹 모델링 n=6 렌즈 (BT-204) — n=6: wave_n:variant_sigma:immunity_tau:vaccine_J2:intervention_phi:tier_3
pub struct PandemicModelingLens;

impl Lens for PandemicModelingLens {
    fn name(&self) -> &'static str { "PandemicModelingLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 4.0, 24.0, 2.0, 3.0];
        ...
        r.insert("pandemic_modeling_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_pandemic_modeling_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = PandemicModelingLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("pandemic_modeling_score"));
    }
}
