...
/// 나노기술 n=6 렌즈 (BT-87,BT-88) — n=6: shell_n:diameter_nm_phi:layer_sigma:bond_tau:J2_atom:precursor_3
pub struct NanotechnologyLens;

impl Lens for NanotechnologyLens {
    fn name(&self) -> &'static str { "NanotechnologyLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 2.0, 12.0, 4.0, 24.0, 3.0];
        ...
        r.insert("nanotechnology_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_nanotechnology_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = NanotechnologyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("nanotechnology_score"));
    }
}
