...
/// 건축 구조 n=6 렌즈 (BT-129) — n=6 연결: floor_n:column_sigma:bay_tau:cantilever_phi:safety_3:story_J2
pub struct BuildingStructureLens;

impl Lens for BuildingStructureLens {
    fn name(&self) -> &'static str { "BuildingStructureLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 4.0, 2.0, 3.0, 24.0];
        ...
        r.insert("building_structure_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_building_structure_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = BuildingStructureLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("building_structure_score"));
    }
}
