...
/// 망원경 광학 n=6 렌즈 (BT-189) — n=6 연결: mirror_seg_6:focal_ratio_12:wavelength_band_6:filter_4:baffle_n
pub struct TelescopeOpticsLens;

impl Lens for TelescopeOpticsLens {
    fn name(&self) -> &'static str { "TelescopeOpticsLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 4.0, 24.0, 3.0, 2.0];
        ...
        r.insert("telescope_optics_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_telescope_optics_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = TelescopeOpticsLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("telescope_optics_score"));
    }
}
