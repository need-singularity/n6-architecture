...
/// 해양파 n=6 렌즈 (BT-213,BT-343) — n=6 연결: wave_period_6:beaufort_12:tidal_tau:depth_sigma:salinity_6
pub struct OceanWaveLens;

impl Lens for OceanWaveLens {
    fn name(&self) -> &'static str { "OceanWaveLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 4.0, 24.0, 3.0, 48.0];
        ...
        r.insert("ocean_wave_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_ocean_wave_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = OceanWaveLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("ocean_wave_score"));
    }
}
