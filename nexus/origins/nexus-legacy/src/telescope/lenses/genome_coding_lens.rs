...
/// 유전체 코딩 n=6 렌즈 — BT-146, BT-188
/// n=6 연결: 코돈=2^n=64, 아미노산=20=J2-tau, 염색체=J2=24
pub struct GenomeCodingLens;

impl Lens for GenomeCodingLens {
    fn name(&self) -> &'static str { "GenomeCodingLens" }
    ...
    (6.0, "reading_frame"),        ];
        ...
        r.insert("genome_coding_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_genome_coding_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = GenomeCodingLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("genome_coding_score"));
    }
}
