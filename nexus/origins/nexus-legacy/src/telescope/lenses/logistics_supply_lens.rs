...
/// 물류 공급망 n=6 렌즈 (BT-281) — n=6 연결: lead_time_sigma:reorder_n:safety_tau:sku_J2:echelon_phi:container_n
pub struct LogisticsSupplyLens;

impl Lens for LogisticsSupplyLens {
    fn name(&self) -> &'static str { "LogisticsSupplyLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("logistics_supply_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_logistics_supply_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = LogisticsSupplyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("logistics_supply_score"));
    }
}
