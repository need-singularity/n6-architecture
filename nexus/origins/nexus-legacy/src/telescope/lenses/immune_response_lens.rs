...
/// 면역 반응 n=6 렌즈 (BT-155,BT-194) — n=6 연결: antibody_6class:t_cell_tau:b_cell_sigma:cytokine_J2:complement_n:mhc_phi
pub struct ImmuneResponseLens;

impl Lens for ImmuneResponseLens {
    fn name(&self) -> &'static str { "ImmuneResponseLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 4.0, 12.0, 24.0, 3.0, 2.0];
        ...
        r.insert("immune_response_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_immune_response_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = ImmuneResponseLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("immune_response_score"));
    }
}
