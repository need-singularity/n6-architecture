...
/// 공급망 리스크 n=6 렌즈 (BT-281) — n=6: tier_n:buffer_sigma:disruption_tau:recovery_J2:resilience_phi:partner_3
pub struct SupplyChainRiskLens;

impl Lens for SupplyChainRiskLens {
    fn name(&self) -> &'static str { "SupplyChainRiskLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("supply_chain_risk_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_supply_chain_risk_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = SupplyChainRiskLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("supply_chain_risk_score"));
    }
}
