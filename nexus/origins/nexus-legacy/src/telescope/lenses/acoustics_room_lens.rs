...
/// 실내 음향 n=6 렌즈 (BT-108) — n=6: modes_n:rt60_sigma:absorption_phi:frequency_band_6:reflection_tau:nrc_3
pub struct AcousticsRoomLens;

impl Lens for AcousticsRoomLens {
    fn name(&self) -> &'static str { "AcousticsRoomLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 2.0, 4.0, 3.0, 24.0];
        ...
        r.insert("acoustics_room_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_acoustics_room_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = AcousticsRoomLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("acoustics_room_score"));
    }
}
