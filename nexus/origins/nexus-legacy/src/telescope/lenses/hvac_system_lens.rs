...
/// 냉난방 시스템 n=6 렌즈 (BT-322,BT-323) — n=6: cop_phi:stages_n:duct_sigma:compressor_tau:zone_J2:pue_sigma_phi
pub struct HvacSystemLens;

impl Lens for HvacSystemLens {
    fn name(&self) -> &'static str { "HvacSystemLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[2.0, 6.0, 12.0, 4.0, 24.0, 1.2];
        ...
        r.insert("hvac_system_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_hvac_system_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = HvacSystemLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("hvac_system_score"));
    }
}
