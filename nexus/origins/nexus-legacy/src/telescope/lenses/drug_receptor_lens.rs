...
/// 약물 수용체 n=6 렌즈 — BT-185
/// n=6 연결: 배위수=n=6, 도킹사이트=sigma-tau=8, 결합친화도 nM=1/n
pub struct DrugReceptorLens;

impl Lens for DrugReceptorLens {
    fn name(&self) -> &'static str { "DrugReceptorLens" }
    ...
    (24.0, "J2_affinity"),        ];
        ...
        r.insert("drug_receptor_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_drug_receptor_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 12.0 } else { 6.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = DrugReceptorLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("drug_receptor_score"));
    }
}
