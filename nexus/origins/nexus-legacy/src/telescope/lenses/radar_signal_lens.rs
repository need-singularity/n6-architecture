...
/// 레이더 신호 n=6 렌즈 (BT-327) — n=6 연결: pulse_n:dwell_tau:scan_sigma:doppler_J2:beam_phi:fft_48
pub struct RadarSignalLens;

impl Lens for RadarSignalLens {
    fn name(&self) -> &'static str { "RadarSignalLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 4.0, 12.0, 24.0, 2.0, 48.0];
        ...
        r.insert("radar_signal_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_radar_signal_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = RadarSignalLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("radar_signal_score"));
    }
}
