...
/// 열가소성 가공 n=6 렌즈 (BT-131) — n=6: zone_n:residence_tau:pressure_sigma:screw_phi:shot_J2:stage_3
pub struct ThermoplasticProcessLens;

impl Lens for ThermoplasticProcessLens {
    fn name(&self) -> &'static str { "ThermoplasticProcessLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 4.0, 12.0, 2.0, 24.0, 3.0];
        ...
        let score = (n6_matches as f64 / d.max(1) as f64).min(1.0) * 0.5
            + attractor_288 * 0.2 + phi_doubling * 0.3;
        ...
        r.insert("thermoplastic_process_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_thermoplastic_process_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = ThermoplasticProcessLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("thermoplastic_process_score"));
    }
}
