...
/// 마찰학 n=6 렌즈 (BT-131) — n=6: friction_n:wear_tau:lubrication_phi:contact_sigma:fatigue_J2:hardness_3
pub struct TribologyLens;

impl Lens for TribologyLens {
    fn name(&self) -> &'static str { "TribologyLens" }
    ...
    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        ...
        r.insert("tribology_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_tribology_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = TribologyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("tribology_score"));
    }
}
