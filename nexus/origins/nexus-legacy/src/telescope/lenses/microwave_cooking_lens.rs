...
/// 전자레인지 n=6 렌즈 (BT-145) — n=6: frequency_J2_ghz:power_sigma:mode_n:rotation_phi:cycle_tau:resonance_3
pub struct MicrowaveCookingLens;

impl Lens for MicrowaveCookingLens {
    fn name(&self) -> &'static str { "MicrowaveCookingLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[2.45, 12.0, 6.0, 2.0, 4.0, 3.0];
        ...
        r.insert("microwave_cooking_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_microwave_cooking_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = MicrowaveCookingLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("microwave_cooking_score"));
    }
}
