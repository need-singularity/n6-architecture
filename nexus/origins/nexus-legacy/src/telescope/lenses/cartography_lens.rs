...
/// 지도학 n=6 렌즈 (BT-154,BT-191) — n=6: scale_n:projection_tau:layer_phi:grid_sigma:symbol_J2:datum_3
pub struct CartographyLens;

impl Lens for CartographyLens {
    fn name(&self) -> &'static str { "CartographyLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 4.0, 2.0, 12.0, 24.0, 3.0];
        ...
        r.insert("cartography_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_cartography_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = CartographyLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("cartography_score"));
    }
}
