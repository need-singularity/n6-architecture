...
/// 지진 지진파 n=6 렌즈 (BT-203) — n=6 연결: richter_6:p_wave_4:s_wave_6:surface_12:moment_tau:depth_sigma
pub struct EarthquakeSeismicLens;

impl Lens for EarthquakeSeismicLens {
    fn name(&self) -> &'static str { "EarthquakeSeismicLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 4.0, 12.0, 3.0, 24.0, 2.0];
        ...
        r.insert("earthquake_seismic_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_earthquake_seismic_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = EarthquakeSeismicLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("earthquake_seismic_score"));
    }
}
