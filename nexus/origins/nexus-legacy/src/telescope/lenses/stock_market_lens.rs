...
/// 주식시장 n=6 렌즈 (BT-338,BT-147) — n=6 연결: ma_6:ma_12:ma_24:rsi_tau:bollinger_sigma:vix_phi
pub struct StockMarketLens;

impl Lens for StockMarketLens {
    fn name(&self) -> &'static str { "StockMarketLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 12.0, 24.0, 4.0, 3.0, 2.0];
        ...
        r.insert("stock_market_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_stock_market_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = StockMarketLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("stock_market_score"));
    }
}
