...
/// 신경망 뇌 n=6 렌즈 (BT-132,BT-254) — n=6 연결: cortex_layers_6:gamma_40hz:theta_6hz:sigma_12hz:tau_4:J2_24hz
pub struct BrainNeuralLens;

impl Lens for BrainNeuralLens {
    fn name(&self) -> &'static str { "BrainNeuralLens" }
    fn category(&self) -> &'static str { "T1" }

    fn scan(&self, data: &[f64], n: usize, d: usize, _shared: &SharedData) -> LensResult {
        if n < 4 || d == 0 { return HashMap::new(); }
        let dom_consts: &[f64] = &[6.0, 40.0, 6.0, 12.0, 4.0, 24.0];
        ...
        r.insert("brain_neural_score".to_string(), vec![score]);
...
mod tests {
    use super::*;
    #[test]
    fn test_brain_neural_basic() {
        let n = 8; let d = 2;
        let data: Vec<f64> = (0..n*d).map(|i| if i%d==0 { 6.0 } else { 12.0 }).collect();
        let shared = SharedData::compute(&data, n, d);
        let result = BrainNeuralLens.scan(&data, n, d, &shared);
        assert!(result.contains_key("brain_neural_score"));
    }
}
