...
fn run_graph(domain: Option<String>, format: GraphFormat) -> Result<(), String> {
    ...
    if filtered_nodes.is_empty() {
        println!();
        println!("  (no nodes matching filter -- run 'nexus evolve <domain>' to populate)");
        return Ok(());
    }
...
fn run_history(domain: &str) -> Result<(), String> {
    ...
    let records = recorder::load_records("nexus_history", domain);

    if records.is_empty() {
        println!();
        println!("  No scan history for domain '{}'.", domain);
        println!("  Run 'nexus scan {}' or 'nexus evolve {}' to start.", domain, domain);
        return Ok(());
    }
    ...
    let results = match mode {
        ...
        ExperimentMode::Single(ref type_name) => {
            let exp_type = ExperimentType::from_str(type_name)
                .ok_or_else(|| format!("Unknown experiment type: '{}'. Use 'nexus experiment all <target>' to see all types.", type_name))?;
...
fn print_help() {
    ...
    println!("Usage: nexus <command> [options]");
    println!();
    println!("Commands:");
