...
pub fn parse_args(args: &[String]) -> Result<CliCommand, String> {
    ...
    match sub {
        ...
        other => Err(format!("Unknown command: '{}'. Run 'nexus help' for usage.", other)),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_parse_scan() {
        let cmd = parse_args(&args("nexus scan physics --lenses consciousness,topology")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Scan {
    ...
    fn test_parse_scan_full() {
        let cmd = parse_args(&args("nexus scan biology --full")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Scan {
    ...
    fn test_parse_verify() {
        let cmd = parse_args(&args("nexus verify 12.0")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Verify {
                value: 12.0,
    ...
    fn test_parse_verify_tolerance() {
        let cmd = parse_args(&args("nexus verify 11.9 --tolerance 0.05")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Verify {
                value: 11.9,
    ...
    #[test]
    fn test_parse_help() {
        assert_eq!(parse_args(&args("nexus help")).unwrap(), CliCommand::Help);
        assert_eq!(parse_args(&args("nexus")).unwrap(), CliCommand::Help);
        assert_eq!(parse_args(&args("nexus --help")).unwrap(), CliCommand::Help);
    }
    ...
    fn test_parse_dashboard() {
        assert_eq!(
            parse_args(&args("nexus dashboard")).unwrap(),
    ...
    fn test_parse_dashboard_html() {
        assert_eq!(
            parse_args(&args("nexus dashboard --html")).unwrap(),
    ...
    fn test_parse_dashboard_html_output() {
        assert_eq!(
            parse_args(&args("nexus dashboard --html --output report.html")).unwrap(),
    ...
    fn test_parse_bench() {
        assert_eq!(
            parse_args(&args("nexus bench")).unwrap(),
            CliCommand::Bench
        );
    }
    ...
    fn test_parse_evolve() {
        let cmd = parse_args(&args("nexus evolve physics --max-cycles 10")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Evolve {
                domain: "physics".to_string(),
    ...
    fn test_parse_evolve_seeds() {
        let cmd = parse_args(&args("nexus evolve cosmology --seeds BT-97,BT-98 --max-cycles 3")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Evolve {
    ...
    fn test_parse_lenses() {
        let cmd = parse_args(&args("nexus lenses --category core")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Lenses {
    ...
    fn test_parse_lenses_all() {
        let cmd = parse_args(&args("nexus lenses")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Lenses {
                category: None,
    ...
    fn test_parse_lenses_search() {
        let cmd = parse_args(&args("nexus lenses --search quantum")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Lenses {
                category: None,
    ...
    fn test_parse_lenses_domain() {
        let cmd = parse_args(&args("nexus lenses --domain physics")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Lenses {
                category: None,
    ...
    fn test_parse_lenses_count() {
        let cmd = parse_args(&args("nexus lenses --count")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Lenses {
                category: None,
    ...
    fn test_parse_lenses_complementary() {
        let cmd = parse_args(&args("nexus lenses --complementary wave")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Lenses {
                category: None,
    ...
    fn test_parse_lenses_export() {
        let cmd = parse_args(&args("nexus lenses --export json")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Lenses {
                category: None,
    ...
    #[test]
    fn test_parse_graph() {
        let cmd = parse_args(&args("nexus graph --domain physics --format dot")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Graph {
                domain: Some("physics".to_string()),
    ...
    fn test_parse_history() {
        let cmd = parse_args(&args("nexus history energy")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::History {
    ...
    fn test_parse_recommend() {
        let cmd = parse_args(&args("nexus recommend biology")).unwrap();
        assert_eq!(
            cmd,
    ...
    fn test_parse_ingest_default() {
        let cmd = parse_args(&args("nexus ingest")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Ingest {
                sources: vec![],
    ...
    fn test_parse_ingest_with_sources() {
        let cmd = parse_args(&args("nexus ingest --source /tmp/proj1 --source /tmp/proj2 --verbose")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Ingest {
    ...
    fn test_parse_ingest_with_config() {
        let cmd = parse_args(&args("nexus ingest --config /tmp/projects.json")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Ingest {
                sources: vec![],
    ...
    fn test_unknown_command() {
        assert!(parse_args(&args("nexus foobar")).is_err());
    }

    #[test]
    fn test_scan_missing_domain() {
        assert!(parse_args(&args("nexus scan")).is_err());
    }

    #[test]
    fn test_verify_bad_number() {
        assert!(parse_args(&args("nexus verify abc")).is_err());
    }

    #[test]
    fn test_parse_auto_default() {
        let cmd = parse_args(&args("nexus auto physics")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Auto {
                domain: "physics".to_string(),
    ...
    fn test_parse_auto_with_options() {
        let cmd = parse_args(&args("nexus auto biology --meta-cycles 3 --ouroboros-cycles 12")).unwrap();
        assert_eq!(
            cmd,
            CliCommand::Auto {
                domain: "biology".to_string(),
    ...
    fn test_parse_auto_missing_domain() {
        assert!(parse_args(&args("nexus auto")).is_err());
    }
}
