...
- **임상 분산**: "디코더 슬라이딩 12 ms"는 임상 그룹마다 10~16 ms 범위 (Nuyujukian et al. 2018). 12 ms 는 BrainGate2 보고치이며 본 논문의 n=6 매핑(σ(6)=12)은 "중앙값 고정" 해석일 뿐, 그룹 간 변동은 MISS 로 reality_map.json 에 기록된다.
- **자극 펄스폭 연속분포**: 200~250 μs 범위 (CLOSE). 정수 유도식으로 환원되지 않는 연속량이므로 σ/φ/τ 대조에서 제외하고 appendix MISS 로 분류.
- **소표본 대조**: 본 문서의 "n=6 vs 대조군(n=4,5,7,8)" 비교는 인접 4개 정수에 한정. 더 큰 정수 범위(1~1000)는 별도 `verify_hexa_neuro.py` 에서 수행하나, BCI 도메인 지식의 Small Integer Bias(≤24)는 원리적으로 제거 불가.
- **해석 선택 편향**: Utah Array 100 전극의 기본 단위를 "10x10 x 6 타일 = 3600" 로 재해석한 것은 Normann 원 설계가 아니라 본 논문의 후행 프레이밍이다. 원 사양(100)은 σ(6)=12 와 직결되지 않으므로 해당 유도는 "설계 제안"으로만 간주.
- **시계열 일반화 불가**: 감마파 30~80 Hz 의 하한/중심 두 값만 σ·sopfr 도출과 일치하며, 상한 80 Hz 는 n=6 함수 조합으로 정수 도출되지 않는다. 이는 MISS 로 명시.
- **기술 성숙도**: 의식 업로드와 달리 BCI 는 이미 임상 단계 — Mk.I 는 진짜(10년). 그러나 6144 채널 grid 는 현재 Neuralink N1 (1024) 의 6배로, 전력·열·FFC 연결 제약에서 실증 미검증.
