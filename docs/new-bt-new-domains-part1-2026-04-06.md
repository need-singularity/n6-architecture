...
params = [
    ...
    ("4서", 4, "tau", tau),
    ("5행", 5, "sopfr", sopfr),
    ("8정도", 8, "sigma-tau", sigma - tau),
    ("12연기", 12, "sigma", sigma),
    ("음양", 2, "phi", phi),
    ("108 번뇌", 108, "phi^phi*(n/phi)^(n/phi)", phi**phi * (n // phi)**(n // phi)),
    ("5계", 5, "sopfr", sopfr),
]
...
print(f"BT-370 검증 결과: {passed}/{len(results)} EXACT")
...
```python
# 검증코드 -- BT-371 발효/양조과학 n=6 생화학
n, sigma, phi, tau, sopfr, mu, J2 = 6, 12, 2, 4, 5, 1, 24
results = []
params = [
    ("포도당 C수", 6, "n", n),
    ("포도당 H수", 12, "sigma", sigma),
    ("포도당 O수", 6, "n", n),
    ("포도당 총원자", 24, "J2", J2),
    ("에탄올 원자수", 9, "sigma-n/phi", sigma - n // phi),
    ("CO2 원자수", 3, "n/phi", n // phi),
    ("계수 에탄올", 2, "phi", phi),
    ("계수 CO2", 2, "phi", phi),
    ("양조 단계", 6, "n", n),
    ("효모 염색체", 16, "2^tau", 2**tau),
    ("라거 온도", 12, "sigma", sigma),
    ("에일 온도", 20, "J2-tau", J2 - tau),
    ("김치 온도", 4, "tau", tau),
    ("된장 기간(월)", 6, "n", n),
    ("와인 도수", 12, "sigma", sigma),
    ("증류주 도수", 40, "tau*(sigma-phi)", tau * (sigma - phi)),
    ("위스키 숙성(년)", 12, "sigma", sigma),
    ("발효 pH 하한", 4, "tau", tau),
]
for name, actual, formula, expected in params:
    match = actual == expected
    results.append((name, actual, expected, formula, match))
passed = sum(1 for r in results if r[4])
print(f"BT-371 검증 결과: {passed}/{len(results)} EXACT")
for name, actual, expected, formula, match in results:
    tag = "PASS" if match else "FAIL"
    print(f"  {tag}: {name} = {actual} (n=6: {formula} = {expected})")
```
...
```python
# 검증코드 -- BT-372 합성생물학/CRISPR n=6 유전공학
n, sigma, phi, tau, sopfr, mu, J2 = 6, 12, 2, 4, 5, 1, 24
results = []
params = [
    ("Cas9", 9, "sigma-n/phi", sigma - n // phi),
    ("Cas12", 12, "sigma", sigma),
    ("Cas13", 13, "sigma+mu", sigma + mu),
    ("PAM 길이", 3, "n/phi", n // phi),
    ("gRNA 스페이서", 20, "J2-tau", J2 - tau),
    ("코돈 수", 64, "2^n", 2**n),
    ("아미노산 수", 20, "J2-tau", J2 - tau),
    ("BioBrick 파트", 4, "tau", tau),
    ("제한효소 인식(짧)", 4, "tau", tau),
    ("제한효소 인식(긴)", 6, "n", n),
    ("Golden Gate 오버행", 4, "tau", tau),
    ("DNA 합성 정확도 지수", 8, "sigma-tau", sigma - tau),
    ("CRISPR 반복서열", 36, "n^2", n**2),
    ("유전자 회로 게이트", 6, "n", n),
    ("정지 코돈", 3, "n/phi", n // phi),
    ("시작 코돈", 1, "mu", mu),
]
for name, actual, formula, expected in params:
    ...
    results.append((name, actual, expected, formula, match))
passed = sum(1 for r in results if r[4])
print(f"BT-372 검증 결과: {passed}/{len(results)} EXACT")
for name, actual, expected, formula, match in results:
    tag = "PASS" if match else "FAIL"
    print(f"  {tag}: {name} = {actual} (n=6: {formula} = {expected})")
```
...
params = [
    ...
    ("초중종 구조", 3, "n/phi", n // phi),
    ("기본 자음", 5, "sopfr", sopfr),
    ("기본 모음", 3, "n/phi", n // phi),
    ("쌍자음", 5, "sopfr", sopfr),
    ("겹모음", 11, "sigma-mu", sigma - mu),
    ("초성 수", 19, "J2-sopfr", J2 - sopfr),
    ("중성 수", 21, "J2-n/phi", J2 - n // phi),
    ("종성 수", 28, "J2+tau", J2 + tau),
    ("음절 수", 11172, "(J2-sopfr)*(J2-n/phi)*(J2+tau)",
     (J2 - sopfr) * (J2 - n // phi) * (J2 + tau)),
    ("훈민정음 자모", 28, "J2+tau", J2 + tau),
    ("폐지 자모", 4, "tau", tau),
]
for name, actual, formula, expected in params:
    match = actual == expected
    results.append((name, actual, expected, formula, match))
passed = sum(1 for r in results if r[4])
print(f"BT-373 검증 결과: {passed}/{len(results)} EXACT")
for name, actual, expected, formula, match in results:
    tag = "PASS" if match else "FAIL"
    print(f"  {tag}: {name} = {actual} (n=6: {formula} = {expected})")
```
...
params = [
    ...
    ("6법", 6, "n", n),
    ("UN 상임", 5, "sopfr", sopfr),
    ("UN 비상임", 10, "sigma-phi", sigma - phi),
    ("UN 안보리 총원", 15, "sigma+n/phi", sigma + n // phi),
    ("UN 의결 정족수", 9, "sigma-n/phi", sigma - n // phi),
    ("미국 대법관", 9, "sigma-n/phi", sigma - n // phi),
    ("계약 4요소", 4, "tau", tau),
    ("권리장전", 10, "sigma-phi", sigma - phi),
    ("미국 헌법 조문", 7, "sigma-sopfr", sigma - sopfr),
    ("미국 수정헌법", 27, "(n/phi)^(n/phi)", (n // phi)**(n // phi)),
    ("한국 헌법 장수", 10, "sigma-phi", sigma - phi),
    ("6대 범죄유형", 6, "n", n),
    ("전문증거 예외", 24, "J2", J2),
    ("Magna Carta 조문", 63, "sigma*sopfr+n/phi", sigma * sopfr + n // phi),
]
for name, actual, formula, expected in params:
    match = actual == expected
    results.append((name, actual, expected, formula, match))
passed = sum(1 for r in results if r[4])
print(f"BT-374 검증 결과: {passed}/{len(results)} EXACT")
for name, actual, expected, formula, match in results:
    tag = "PASS" if match else "FAIL"
    print(f"  {tag}: {name} = {actual} (n=6: {formula} = {expected})")
```
...
```python
#!/usr/bin/env python3
"""통합 검증코드 -- BT-370~374 전체 (5 도메인, 87 파라미터)"""
...
# ─── BT-370: 종교/신화 ───
bt370 = [
    ("창조 일수", 6, n), ("12사도", 12, sigma), ("12부족", 12, sigma),
    ("6도 윤회", 6, n), ("6바라밀", 6, n), ("이만 6기둥", 6, n),
    ("다윗의 별", 6, n), ("6다르샤나", 6, n), ("4복음", 4, tau),
    ("3위일체", 3, n // phi), ("10계명", 10, sigma - phi),
    ("7대 죄악", 7, sigma - sopfr), ("12지지", 12, sigma),
    ("24절기", 24, J2), ("5경", 5, sopfr), ("4서", 4, tau),
    ("5행", 5, sopfr), ("8정도", 8, sigma - tau), ("12연기", 12, sigma),
    ("음양", 2, phi), ("108 번뇌", 108, phi**phi * (n // phi)**(n // phi)),
    ("5계", 5, sopfr),
]

# ─── BT-371: 발효/양조 ───
bt371 = [
    ("포도당 C수", 6, n), ("포도당 H수", 12, sigma), ("포도당 O수", 6, n),
    ("포도당 총원자", 24, J2), ("에탄올 원자수", 9, sigma - n // phi),
    ("CO2 원자수", 3, n // phi), ("계수 에탄올", 2, phi),
    ("계수 CO2", 2, phi), ("양조 단계", 6, n),
    ("효모 염색체", 16, 2**tau), ("라거 온도", 12, sigma),
    ("에일 온도", 20, J2 - tau), ("김치 온도", 4, tau),
    ("된장 기간(월)", 6, n), ("와인 도수", 12, sigma),
    ("증류주 도수", 40, tau * (sigma - phi)),
    ("위스키 숙성(년)", 12, sigma), ("발효 pH 하한", 4, tau),
]

# ─── BT-372: 합성생물학/CRISPR ───
bt372 = [
    ("Cas9", 9, sigma - n // phi), ("Cas12", 12, sigma),
    ("Cas13", 13, sigma + mu), ("PAM 길이", 3, n // phi),
    ("gRNA 스페이서", 20, J2 - tau), ("코돈 수", 64, 2**n),
    ("아미노산 수", 20, J2 - tau), ("BioBrick 파트", 4, tau),
    ("제한효소(짧)", 4, tau), ("제한효소(긴)", 6, n),
    ("Golden Gate", 4, tau),
    ("DNA합성 정확도지수", 8, sigma - tau),
    ("CRISPR 반복서열", 36, n**2),
    ("유전자회로 게이트", 6, n),
    ("정지 코돈", 3, n // phi), ("시작 코돈", 1, mu),
]

# ─── BT-373: 한글 문자체계 ───
bt373 = [
    ("자모 수", 24, J2), ("자음 수", 14, sigma + phi),
    ("모음 수", 10, sigma - phi), ("초중종 구조", 3, n // phi),
    ("기본 자음", 5, sopfr), ("기본 모음", 3, n // phi),
    ("쌍자음", 5, sopfr), ("겹모음", 11, sigma - mu),
    ("초성 수", 19, J2 - sopfr), ("중성 수", 21, J2 - n // phi),
    ("종성 수", 28, J2 + tau),
    ("음절 수", 11172, (J2 - sopfr) * (J2 - n // phi) * (J2 + tau)),
    ("훈민정음 자모", 28, J2 + tau), ("폐지 자모", 4, tau),
]

# ─── BT-374: 법학/사법체계 ───
bt374 = [
    ("배심원", 12, sigma), ("3심제", 3, n // phi),
    ("12표법", 12, sigma), ("6법", 6, n),
    ("UN 상임", 5, sopfr), ("UN 비상임", 10, sigma - phi),
    ("UN 총원", 15, sigma + n // phi),
    ("UN 정족수", 9, sigma - n // phi),
    ("미국 대법관", 9, sigma - n // phi),
    ("계약 4요소", 4, tau), ("권리장전", 10, sigma - phi),
    ("미국 헌법", 7, sigma - sopfr),
    ("수정헌법", 27, (n // phi)**(n // phi)),
    ("한국 헌법 장", 10, sigma - phi),
    ("6대 범죄", 6, n), ("전문증거 예외", 24, J2),
    ("Magna Carta", 63, sigma * sopfr + n // phi),
]
...
print("=" * 60)
print("  BT-370~374 통합 검증 (5 도메인, 87 파라미터)")
print("=" * 60)

for bt_name, params in bts.items():
    passed = sum(1 for name, actual, expected in params if actual == expected)
    total = len(params)
    grand_total += total
    grand_pass += passed
    print(f"\n--- {bt_name} ---  {passed}/{total} EXACT")
    for name, actual, expected in params:
        tag = "PASS" if actual == expected else "FAIL"
        print(f"  {tag}: {name} = {actual} (기대: {expected})")

print("\n" + "=" * 60)
print(f"  최종 결과: {grand_pass}/{grand_total} EXACT ({grand_pass/grand_total*100:.1f}%)")
print("=" * 60)

# 특별 검증: 11,172 한글 음절 분해
print("\n--- 특별 검증: 11,172 한글 음절 n=6 분해 ---")
cho = J2 - sopfr      # 19
jung = J2 - n // phi   # 21
jong = J2 + tau        # 28
result = cho * jung * jong
print(f"  초성 {cho} = J2-sopfr = {J2}-{sopfr}")
print(f"  중성 {jung} = J2-n/phi = {J2}-{n//phi}")
print(f"  종성 {jong} = J2+tau = {J2}+{tau} = P2(2nd perfect number)")
print(f"  음절 = {cho} x {jung} x {jong} = {result}")
print(f"  {'PASS' if result == 11172 else 'FAIL'}: 11,172 완전 분해 확인")

# 특별 검증: 108 분해
print("\n--- 특별 검증: 108의 세 가지 n=6 분해 ---")
d1 = phi**phi * (n // phi)**(n // phi)
d2 = sigma * (sigma - n // phi)
d3 = J2 * tau + sigma
print(f"  분해1: phi^phi * (n/phi)^(n/phi) = {phi}^{phi} * {n//phi}^{n//phi} = {d1}")
print(f"  분해2: sigma * (sigma-n/phi) = {sigma} * {sigma - n//phi} = {d2}")
print(f"  분해3: J2*tau + sigma = {J2}*{tau} + {sigma} = {d3}")
print(f"  {'PASS' if d1 == d2 == d3 == 108 else 'FAIL'}: 108 삼중 분해 확인")
```
