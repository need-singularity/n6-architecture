...
# ═══ I. 수면방추/K-복합체 (6) ═══
check("spindle_center_Hz",  sigma,        12,     "σ=12 Hz 방추 중심",       "Spindle")
check("spindle_low_Hz",     sigma-phi,    10,     "σ-φ=10 Hz 방추 하한",     "Spindle")
check("spindle_high_Hz",    sigma+phi,    14,     "σ+φ=14 Hz 방추 상한",     "Spindle")
check("spindle_dur_s",      phi,          2,      "φ=2초 방추 지속시간",     "Spindle")
check("kcomplex_uV",        sigma**2,     144,    "σ²=144 μV K-복합체 진폭", "Spindle")
check("kcomplex_dur_s",     mu,           1,      "μ=1초 K-복합체 지속",     "Spindle")

# ═══ J. 뇌파 대역 경계 (10) ═══
check("delta_low_Hz",   mu/phi,          0.5,    "μ/φ=0.5 Hz δ 하한",       "BandEdge", tol=0.01)
check("delta_high_Hz",  tau,             4,      "τ=4 Hz δ 상한",            "BandEdge")
check("theta_low_Hz",   tau,             4,      "τ=4 Hz θ 하한",            "BandEdge")
check("theta_high_Hz",  sigma-tau,       8,      "σ-τ=8 Hz θ 상한",          "BandEdge")
check("alpha_low_Hz",   sigma-tau,       8,      "σ-τ=8 Hz α 하한",          "BandEdge")
check("alpha_high_Hz",  sigma,           12,     "σ=12 Hz α 상한",           "BandEdge")
check("beta_low_Hz",    sigma,           12,     "σ=12 Hz β 하한",           "BandEdge")
check("beta_high_Hz",   n*sopfr,         30,     "n·sopfr=30 Hz β 상한",     "BandEdge")
check("gamma_low_Hz",   n*sopfr,         30,     "n·sopfr=30 Hz γ 하한",     "BandEdge")
check("gamma_high_Hz",  (sigma-phi)**2,  100,    "(σ-φ)²=100 Hz γ 상한",     "BandEdge")

# ═══ K. 신경화학/일주기 (10) ═══
check("melatonin_onset_h",  phi,         2,      "φ=2시간 일몰후 멜라토닌",  "Neurochem")
check("melatonin_peak_h",   n//phi,      3,      "n/φ=3시 멜라토닌 피크",    "Neurochem")
check("cortisol_nadir_h",   phi,         2,      "φ=2시 코르티솔 최저",      "Neurochem")
check("temp_nadir_h",       tau,         4,      "τ=4시 체온 최저",          "Neurochem")
check("adenosine_halflife", n,           6,      "n=6시간 아데노신 반감기",  "Neurochem")
check("gaba_inhibit_pct",   sigma*sopfr, 60,     "σ·sopfr=60% GABA 억제",   "Neurochem")
check("serotonin_enzymes",  phi,         2,      "φ=2 전환 효소(NAT+HIOMT)","Neurochem")
check("wake_pressure_h",    sigma+tau,   16,     "σ+τ=16시간 각성 압력 포화","Neurochem")
check("basic_emotions",     n,           6,      "n=6 기본 감정(Ekman)",     "Neurochem")
check("brain_regions",      n,           6,      "n=6 뇌 영역(BT-136)",     "Neurochem")

# ═══ L. 사이클 미세구조 (6) ═══
check("cycle_length_min",   n*(sigma+n//phi), 90, "n·(σ+n/φ)=90분 사이클",  "MicroStruct")
check("first_rem_min",      sigma-phi,   10,     "σ-φ=10분 첫 REM",         "MicroStruct")
check("last_rem_min",       sigma*sopfr, 60,     "σ·sopfr=60분 마지막 REM", "MicroStruct")
check("n3_total_min",       sigma*sopfr, 60,     "σ·sopfr=60분 총 N3",      "MicroStruct")
check("rem_ratio",          120/360,     1/3,    "μ/(n/φ)=1/3 REM/총수면",  "MicroStruct", tol=0.001)
check("n3_ratio",           60/360,      1/n,    "μ/n=1/6 N3/총수면",       "MicroStruct", tol=0.001)
