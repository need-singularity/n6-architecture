...
**90%+ 도달 (물리 한계 근접, 🛸10 근거)**:
1. Lawson 삼중곱 (Mk.I 280%, Mk.II >1000%) — 이미 한계 초과
2. Troyon beta_N = 3.5 (100% EXACT) — n=6가 MHD 한계를 정확히 적중
3. 열효율 50% (sCO₂ 실용 한계의 91%) — 실용 최적에 근접
4. Greenwald 밀도 (105~170%) — 한계 초과 운전 필요

**50~90% 도달 (성장 여지 있음)**:
5. TBR = 7/6 (실용 한계의 83%) — 적정 마진 보유
6. 자기장 B_T=12T (실증 한계의 60%) — 의도적 보수 설계
7. Q = 30 (실용 상한의 60%) — 상업 최적 구간

**50% 미만 (대폭 마진)**:
8. 벽 부하 (SiC 한계의 7~27%) — 대형 설계가 자연 분산
...
### 11.4 🛸10 도달을 위한 필요 조건

현재 HEXA-FUSION 진화 체인은 🛸5 수준이다. 🛸10(물리적 한계 도달)을 위해서는:
...
```
  ...
  │  필요 조건                          │ 현재 상태    │ 격차        │
  ├──────────────────────────────────────────────────────────────────┤
  │  1. Mk.I 건설 + 운전 데이터 확보   │ 미시작       │ 10~15년     │
  │  2. Q≥10 실험적 달성               │ 미달성       │ SPARC 대기  │
  │  3. Mk.II 상업 발전소 운영         │ 미시작       │ 20~25년     │
  │  4. 전 파라미터 실험 검증           │ 0/8 검증     │ 전수 미검증 │
  │  5. 재료(SiC/SiC) 수명 실증        │ 실험실 단계  │ 5~10년     │
  │  6. 삼중수소 경제 실증             │ 미시작       │ Mk.I 이후   │
  │  7. 밀도 초과 운전 기술 확보        │ 부분 실증    │ 3~5년      │
  │  8. 양산 + 비용 경쟁력             │ 미시작       │ 30+년      │
  └──────────────────────────────────────────────────────────────────┘
  
  🛸10 도달 예상 시점: 2070+ (Mk.III 안정 운전 이후)
  현재 🛸 등급: 5 (상세 설계 + BT + DSE 완료)
  
  핵심 메시지:
    n=6 설계는 물리 한계의 올바른 위치에 파라미터를 배치한다.
    beta_N = 3.5 EXACT, eta = 50% 근최적, B_T = 12T 보수적.
    물리적 한계 "자체"에는 문제가 없다.
    병목은 물리가 아닌 "실험적 검증 + 공학적 양산"이다.
```
...
```
  ...
  🛸10 도달 여부:
    - 물리적 한계의 위치 파악: 완료 (beta_N, Lawson, Greenwald 모두 정량화)
    - 물리적 한계 내 최적 설계: 완료 (8개 파라미터 전수 분석)
    - 실험적 검증: 미완 (🛸5→🛸10의 유일한 격차)
    
  최종 판정: 설계 수준에서는 물리 한계에 도달했다.
  🛸10은 실험적 검증 완료 시 달성. 현재 🛸5.
```
