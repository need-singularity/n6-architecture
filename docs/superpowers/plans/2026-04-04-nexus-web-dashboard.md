...
**Architecture:** Axum HTTP 서버가 `~/.nexus/` JSON 파일과 `growth.log`를 직접 읽어 API로 서빙. 프론트엔드는 단일 HTML에 Chart.js + vanilla JS. SSE로 로그 스트리밍. 프로세스 제어는 `Command::new("bash")`.

**Tech Stack:** Rust (Axum + Tokio + Serde + tower-http), Chart.js 4.x (CDN), vanilla JS

**Spec:** `docs/superpowers/specs/2026-04-04-nexus-web-dashboard-design.md`
...
│   ├── data.rs            — ~/.nexus/ JSON 읽기 + measure.sh 메트릭 파싱
...
**Files:**
- Create: `tools/nexus-dashboard/Cargo.toml`
- Create: `tools/nexus-dashboard/src/main.rs`
...
Run: `cd tools/nexus-dashboard && ~/.cargo/bin/cargo build --release 2>&1 | tail -5`
Expected: `Compiling nexus-dashboard` 성공

- [ ] **Step 4: 서버 시작 + curl 테스트**

Run: `./target/release/nexus-dashboard &` then `curl -s http://localhost:6600`
...
git add tools/nexus-dashboard/
git commit -m "feat: nexus-dashboard 최소 Axum 서버 (port 6600)"
```
...
**Files:**
- Create: `tools/nexus-dashboard/src/targets.rs`
- Create: `tools/nexus-dashboard/src/tests/targets_test.rs`
- Modify: `tools/nexus-dashboard/src/main.rs` (mod 선언 추가)
...
Run: `cd tools/nexus-dashboard && ~/.cargo/bin/cargo test -- --nocapture 2>&1 | tail -10`
Expected: `test result: ok. 3 passed`

- [ ] **Step 5: 커밋**

```bash
git add tools/nexus-dashboard/src/targets.rs tools/nexus-dashboard/src/tests/
git commit -m "feat: 15차원 목표값 정의 (targets.rs) + 테스트"
```

---

### Task 3: data.rs — ~/.nexus/ JSON 읽기

**Files:**
- Create: `tools/nexus-dashboard/src/data.rs`
- Create: `tools/nexus-dashboard/src/tests/data_test.rs`
- Modify: `tools/nexus-dashboard/src/main.rs` (mod 추가)
...
#[cfg(test)]
mod tests {
    use crate::data::{nexus_dir, read_json_file, read_growth_log, read_daemon_log_lines};

    #[test]
    fn test_nexus_dir_exists() {
        let dir = nexus_dir();
        // CI에서는 없을 수 있으므로 경로 형식만 확인
        assert!(dir.ends_with(".nexus"));
    }
...
pub fn read_json_file(filename: &str) -> Result<Value, String> {
    let path = nexus_dir().join(filename);
...
pub fn read_growth_log() -> Vec<Value> {
    let path = repo_root()
        .join("tools/nexus/scripts/growth_log.jsonl");
...
pub fn read_daemon_log_lines(max_lines: usize) -> Vec<String> {
    let path = nexus_dir().join("growth.log");
...
```bash
git add tools/nexus-dashboard/src/data.rs tools/nexus-dashboard/src/tests/data_test.rs
git commit -m "feat: data.rs — ~/.nexus/ JSON 읽기 + 로그 파싱"
```
...
**Files:**
- Create: `tools/nexus-dashboard/src/routes/mod.rs`
- Create: `tools/nexus-dashboard/src/routes/dimensions.rs`
- Create: `tools/nexus-dashboard/src/routes/history.rs`
- Create: `tools/nexus-dashboard/src/routes/recent.rs`
- Create: `tools/nexus-dashboard/src/routes/benchmark.rs`
- Modify: `tools/nexus-dashboard/src/main.rs` (라우트 등록)
...
현재 메트릭을 measure.sh 방식으로 계산하는 대신, growth.log 마지막 엔트리와 ~/.nexus/ 파일에서 읽기.
...
```bash
git add tools/nexus-dashboard/src/routes/
...
**Files:**
- Create: `tools/nexus-dashboard/src/routes/daemon.rs`
- Modify: `tools/nexus-dashboard/src/routes/mod.rs` (추가)
- Modify: `tools/nexus-dashboard/src/main.rs` (라우트 등록)
...
pub async fn start_daemon() -> Json<serde_json::Value> {
    ...
    let script = repo_root.join("tools/nexus/scripts/nexus_growth_daemon.sh");
...
pub async fn grow_dimension(Path(dimension): Path<String>) -> Json<serde_json::Value> {
    let repo_root = data::repo_root();
    let script = repo_root.join("tools/nexus/scripts/auto_grow.sh");
...
fn check_daemon() -> (bool, Option<u32>) {
    // ps aux에서 nexus_growth_daemon 찾기
    ...
    match output {
        Ok(out) => {
            ...
            for line in stdout.lines() {
                if line.contains("nexus_growth_daemon") && !line.contains("grep") {
...
```bash
git add tools/nexus-dashboard/src/routes/daemon.rs
...
**Files:**
- Create: `tools/nexus-dashboard/src/routes/logs.rs`
- Modify: `tools/nexus-dashboard/src/routes/mod.rs`
- Modify: `tools/nexus-dashboard/src/main.rs`
...
```bash
git add tools/nexus-dashboard/src/routes/logs.rs tools/nexus-dashboard/Cargo.toml
...
**Files:**
- Create: `tools/nexus-dashboard/src/routes/dashboard.rs`
...
```bash
git add tools/nexus-dashboard/src/routes/dashboard.rs
...
**Files:**
- Create: `tools/nexus-dashboard/static/index.html`
...
Run:
```bash
cd tools/nexus-dashboard && ~/.cargo/bin/cargo build --release 2>&1 | tail -5
```
Expected: 성공
...
```bash
git add tools/nexus-dashboard/static/index.html tools/nexus-dashboard/src/routes/dashboard.rs
...
**Files:**
- Modify: `tools/nexus-dashboard/src/main.rs` — 모든 라우트 통합
...
Run: `cd tools/nexus-dashboard && ~/.cargo/bin/cargo test 2>&1 | tail -10`
Expected: 전체 통과

- [ ] **Step 3: 릴리즈 빌드**

Run: `cd tools/nexus-dashboard && ~/.cargo/bin/cargo build --release 2>&1 | tail -3`
Expected: 성공
