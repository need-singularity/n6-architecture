...
데이터 변경분만 재계산:
  ...
  캐시 위치: .cache/nexus/{domain}-{hash}.bin
```
...
```bash
nexus scan <domain>              # 단일 도메인 스캔
nexus scan --cross A B           # 교차 스캔
nexus scan --all                 # 전체 도메인
nexus run --roadmap              # OUROBOROS 무한 루프
nexus run --resume               # 이어하기
nexus verify <id>                # 발견 검증
nexus graph show                 # Discovery Graph 시각화
nexus graph loops                # 닫힌 루프 목록
nexus graph hubs                 # 허브 노드 목록
nexus history <domain>           # 렌즈 히스토리
nexus recommend <domain>         # 최적 렌즈 조합 추천
nexus dashboard                  # 웹 대시보드
nexus watch                      # 파일 변경 감시
nexus calibrate                  # 시스템 검증
nexus benchmark --gpu            # metal-rs 벤치마크
```
