...
`~/.nexus/` 데이터를 시각화하고, 데몬 제어 및 실시간 로그 스트리밍 제공.
...
- **Data source**: `~/.nexus/*.json`, `tools/nexus/scripts/growth_log.jsonl`
- **Port**: localhost:6600
...
- **Start**: `bash tools/nexus/scripts/nexus_growth_daemon.sh --max-cycles 999 --interval 30 --skip-commit &`
- **Stop**: PID from `~/.nexus/daemon.pid` or `ps aux | grep nexus_growth_daemon`
- **Status**: PID 존재 + `/proc/PID` 또는 `kill -0 PID` 확인
- **Manual grow**: `bash tools/nexus/scripts/auto_grow.sh --dimension <dim> --cycles 1`
