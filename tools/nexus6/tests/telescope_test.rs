...
fn test_registry_core_count() {
    ...
    assert_eq!(reg.len(), 717, "Total registry size should be 717 after new()");
}
...
fn test_registry_by_category() {
    ...
    // 695 Extended, no Custom or DomainCombo
    assert_eq!(reg.by_category(LensCategory::Extended).len(), 695);
    ...
    assert_eq!(reg.by_category(LensCategory::Extended).len(), 696);
    assert_eq!(reg.len(), 718);
}
...
fn test_global_lens_name_uniqueness() {
    ...
    assert_eq!(total, 717, "Total should be 22+58+75+103+88+100+58+57+55+63+38 = 717");
...
fn test_registry_total_411() {
    let reg = LensRegistry::new();
    assert_eq!(reg.len(), 717, "Registry should have 717 lenses total");

    let extended = reg.by_category(LensCategory::Extended);
    assert_eq!(extended.len(), 695, "Extended category should have 695 lenses");
}
